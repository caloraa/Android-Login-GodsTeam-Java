...
public class Auth extends AsyncTask<String, Void, String> {
    ...
    protected void onPostExecute(String s) {
        ...
        try {
			...
			getActivity().addInfoAuth("Conectado, Bem-Vindo " + "!", "#32852C");
			...
			try {
				...
				if (data.get("MessageString").toString().equals("Seu UID foi alterado!")){
					getActivity().addInfoAuth("Dispositivo não permitido!", "#FF0000");
               }
			
				if (data.get("MessageString").toString().equals("Usuario ou senha incorretos!")){
					getActivity().addInfoAuth("Usuario ou senha Incorretos", "#FF0000");
				}
				
				if (data.get("MessageString").toString().equals("Usuario Invalido")){
					getActivity().addInfoAuth("Usuario / Senha só devem conter caracteres validos", "#FF0000");
				}
			
				if (data.get("MessageString").toString().equals("Seu login expirou!")){
					getActivity().addInfoAuth("Login Expirado", "#FF0000");
				}
